...
Если вы не знаете, что делать с исключенем, не обрабатывайте его, положитесь на код, который выше. На самом верху программы реализуйте один простой обработчик. Если ваша программа&nbsp;&mdash; это REST-сервис, обработчик
...
Этот код решает несколько задач, поэтому выглядит сложным. Мы не будем обсуждать детали, а сразу перейдём к главному методу `ExtractRestError`. Вызывая этот метод, мы передаём ему исключение, а в ответ ожидаем HTTP-статус и описание ошибок для сериализации
в JSON.
